 ...
 * finlink_nds_audio_ring_* (audio_ring.h/.c) -- the dual-audio-client test
 * category's "Audiosignal wird empfangen und wiedergegeben" check for the
 * NDS client's GC_GBA_LINK audio path. Links audio_ring.c + finlink_core
 * directly (for finlink_read_s16le), same pattern as
...
static void encode_s16le(int16_t value, uint8_t *out) {
    finlink_write_u16le(out, (uint16_t)value);
}

static void test_push_then_pull_fifo_order(void) {
    int16_t backing[8];
    finlink_nds_audio_ring ring;
    finlink_nds_audio_ring_init(&ring, backing, 8);

    uint8_t wire[3 * 2];
    encode_s16le(100, wire + 0);
    encode_s16le(-200, wire + 2);
    encode_s16le(300, wire + 4);
    finlink_nds_audio_ring_push(&ring, wire, 3);

    int16_t out[3];
    finlink_nds_audio_ring_pull(&ring, out, 3);
    CHECK(out[0] == 100);
    CHECK(out[1] == -200);
    CHECK(out[2] == 300);
}
...
static void test_underrun_pads_with_silence(void) {
    ...
    finlink_nds_audio_ring_push(&ring, wire, 2);

    int16_t out[5];
    memset(out, 0xAB, sizeof(out)); /* poison, so a missed silence-fill would be caught */
    finlink_nds_audio_ring_pull(&ring, out, 5);
    CHECK(out[0] == 111);
    CHECK(out[1] == 222);
    CHECK(out[2] == 0);
    CHECK(out[3] == 0);
...
static void test_overflow_drops_tail_not_existing_samples(void) {
    ...
    int16_t backing[4];
    finlink_nds_audio_ring ring;
    finlink_nds_audio_ring_init(&ring, backing, 4);

    uint8_t first[3 * 2];
    encode_s16le(1, first + 0);
    encode_s16le(2, first + 2);
    encode_s16le(3, first + 4);
    finlink_nds_audio_ring_push(&ring, first, 3);

    uint8_t second[5 * 2];
    for (int i = 0; i < 5; i++) {
        encode_s16le((int16_t)(100 + i), second + i * 2);
    }
    finlink_nds_audio_ring_push(&ring, second, 5);

    int16_t out[4];
    finlink_nds_audio_ring_pull(&ring, out, 4);
    CHECK(out[0] == 1);
    CHECK(out[1] == 2);
    CHECK(out[2] == 3);
    ...
    finlink_nds_audio_ring_pull(&ring, drained, 2);
    CHECK(drained[0] == 0);
    CHECK(drained[1] == 0);
}
...
static void test_reset_drops_buffered_audio(void) {
    ...
    finlink_nds_audio_ring_pull(&ring, out, 2);
    CHECK(out[0] == 0);
    CHECK(out[1] == 0);
}
