 ...
 * unison_nds_resolve_language() (language_pref.h/.c) -- the "generell:
...
static void test_explicit_pref_wins_over_personal_data(void) {
    ...
    CHECK(unison_nds_resolve_language(STR_LANG_DE, /* personalDataLanguage (EN) */ 1) == STR_LANG_DE);
    CHECK(unison_nds_resolve_language(STR_LANG_FR, /* personalDataLanguage (ES) */ 5) == STR_LANG_FR);
}

static void test_system_pref_maps_personal_data_language(void) {
    CHECK(unison_nds_resolve_language(-1, /* DE */ 3) == STR_LANG_DE);
    CHECK(unison_nds_resolve_language(-1, /* FR */ 2) == STR_LANG_FR);
    CHECK(unison_nds_resolve_language(-1, /* IT */ 4) == STR_LANG_IT);
    CHECK(unison_nds_resolve_language(-1, /* ES */ 5) == STR_LANG_ES);
}

static void test_system_pref_unsupported_personal_data_language_falls_back_to_english(void) {
    CHECK(unison_nds_resolve_language(-1, /* Japanese */ 0) == STR_LANG_EN);
    CHECK(unison_nds_resolve_language(-1, /* English (itself maps to EN anyway) */ 1) == STR_LANG_EN);
    CHECK(unison_nds_resolve_language(-1, /* Chinese */ 6) == STR_LANG_EN);
    CHECK(unison_nds_resolve_language(-1, /* Unknown/Reserved */ 7) == STR_LANG_EN);
}
