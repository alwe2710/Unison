 ...
 * unison_nds_should_show_video_on_bottom() (screen_choice.h/.c) -- same
 * decision and same reasoning as the 3DS client's
 * shouldShowVideoOnBottomScreen(), see that file's own test for the fuller
 * comment. Links screen_choice.c + unison_core directly, same pattern as
 * Cemu/melonDS's tests/test_unison_messages.cpp. */
...
static void test_gc_gba_link_respects_pref(void) {
    CHECK(unison_nds_should_show_video_on_bottom(false, "GC_GBA_LINK") == false);
    CHECK(unison_nds_should_show_video_on_bottom(true, "GC_GBA_LINK") == true);
}
...
static void test_modern_host_forces_bottom_regardless_of_pref(void) {
    ...
    for (size_t i = 0; i < sizeof(modern_host_types) / sizeof(modern_host_types[0]); i++) {
        CHECK(unison_nds_should_show_video_on_bottom(false, modern_host_types[i]) == true);
        CHECK(unison_nds_should_show_video_on_bottom(true, modern_host_types[i]) == true);
    }
}

static void test_unknown_stream_type_falls_back_to_pref(void) {
    CHECK(unison_nds_should_show_video_on_bottom(false, "SOME_FUTURE_STREAM_TYPE") == false);
    CHECK(unison_nds_should_show_video_on_bottom(true, "SOME_FUTURE_STREAM_TYPE") == true);
}
