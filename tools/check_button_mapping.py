...
all 10 unison_key bits (unison/protocol.h) exactly once -- the
...
mapping is a flat `if (keys & KEY_X) mask |= UNISON_KEY_X;` chain instead,
...
PROTOCOL_H = REPO_ROOT / "core" / "include" / "unison" / "protocol.h"

# The canonical set of button bits every console-client mapping table must
# cover -- derived from unison_key's own definition, not hardcoded here, so
# a future protocol.h change (new button added/removed) can't silently drift
# out of sync with this list.
UNISON_KEY_ENUM_RE = re.compile(r"typedef enum \{(.*?)\} unison_key;", re.DOTALL)
UNISON_KEY_NAME_RE = re.compile(r"\b(UNISON_KEY_[A-Z]+)\b")

TABLE_ENTRY_KEY_RE = re.compile(r"\b(UNISON_KEY_[A-Z]+)\b")


def load_canonical_keys() -> set:
    text = PROTOCOL_H.read_text(encoding="utf-8")
    enum_match = UNISON_KEY_ENUM_RE.search(text)
    if not enum_match:
        sys.exit(f"error: no 'unison_key' enum found in {PROTOCOL_H}")
    return set(UNISON_KEY_NAME_RE.findall(enum_match.group(1)))


def extract_table_keys(path: Path, start_marker: str, end_marker: str) -> list:
    """Returns every UNISON_KEY_* occurrence between the first line
...
def main() -> int:
    ...
    if len(canonical) != 10:
        sys.exit(f"error: expected 10 unison_key entries in {PROTOCOL_H}, found {len(canonical)}: {sorted(canonical)}")
    ...
    print(f"OK: all {len(CLIENTS)} console-clients map all {len(canonical)} unison_key bits exactly once.")
    return 0
