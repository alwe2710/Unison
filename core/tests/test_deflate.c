...
int main(void) {
    /* Round-trip through finlink_inflate_raw() (the only thing that
    ...
    size_t max_size = finlink_deflate_max_size(sizeof(src));
    ...
    finlink_deflate_status dstatus =
        finlink_deflate_raw(src, sizeof(src), compressed, max_size, &compressed_size);
    CHECK(dstatus == FINLINK_DEFLATE_OK);
    CHECK(compressed_size > 0);
    CHECK(compressed_size <= max_size);

    uint8_t decoded[sizeof(src)];
    size_t decoded_size = 0;
    finlink_inflate_status istatus =
        finlink_inflate_raw(compressed, compressed_size, decoded, sizeof(decoded), &decoded_size);
    CHECK(istatus == FINLINK_INFLATE_OK);
    ...
    dstatus = finlink_deflate_raw(src, sizeof(src), tiny_out, sizeof(tiny_out), &tiny_out_size);
    CHECK(dstatus == FINLINK_DEFLATE_ERR);
